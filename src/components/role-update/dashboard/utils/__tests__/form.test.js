...
describe("formatFormListResult", () => {
  const form = {
    ...
    changeType: "Fixed Term transfer to permanent",
    submittedDate: "2021-10-26T20:23:20.711Z",
    submittedBy: "Test User",
    effectiveDate: "2022-01-14T00:00:00.000Z",
    status: "UNEVALUATED"
  };
  ...
  it("should format the results for display on the dashboard", () => {
    ...
    expect(result.changeType).toStrictEqual("Fixed Term transfer to permanent");
    expect(result.status).toStrictEqual("Submitted");
    expect(result.submittedDate).toStrictEqual("27/10/2021");
    expect(result.effectiveDate).toStrictEqual("14/01/2022");
  });
});
