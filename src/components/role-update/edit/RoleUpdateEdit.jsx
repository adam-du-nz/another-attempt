...
const RoleUpdateEdit = props => {
  ...
  useEffect(() => {
    ...
    const fetchFormData = async () => {
      ...
      // Redirect to preview if this form is completed or cancelled
      if (
        roleUpdateFormDto.status === "COMPLETED" ||
        roleUpdateFormDto.status === "CANCELLED"
      ) {
        return history(`/form/role-update/${formId}/preview`);
      }
      ...
      if (
        roleUpdateFormDto.changeApproved ===
  ...
  const shouldDisplayCancelOrBackButton = () => {
    ...
    const backButton = (
      <Button tone="success" onClick={() => history("/form/role-update/list")}>
        Back to Dashboard
      </Button>
    );
  ...
  const approval = form.getValues(
    roleUpdateMetaData.changeType.changeApproved.NAME
  );
  useEffect(() => {
    if (
      approval &&
      approval === roleUpdateMetaData.changeType.changeApproved.OPTIONS[0].name
    ) {
      setChangeApproved(true);
    } else {
