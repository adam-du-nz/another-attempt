...
import { getOnboardingForms } from "../../../apis/kilnBackendApis";
import { sort, tableColumns } from "./config";

const PAGE_SIZE = 20;
const DEFAULT_ACTIVE_SORT = { column: "startDate", descending: true };
...
const OnboardingDashboard = () => {
  ...
  const onSelectMoreOptions = (event, selectedFormId) => {
    if (event === "View") {
      history(`/form/onboarding/${selectedFormId}/preview`);
    }
  };
  ...
  const fetchForms = useCallback(async () => {
    ...
    const resp = await getOnboardingForms({
      page: activePage,
      size: PAGE_SIZE,
    ...
    if (resp.status === HttpStatus.FORBIDDEN) {
      history("/form/onboarding/access-denied", {
        replace: true,
  ...
  return (
    <>
      <div>
        <PageHead title="Onboarding Form Dashboard">
